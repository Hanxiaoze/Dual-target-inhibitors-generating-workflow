...
# them in to the path ~/anaconda3/envs/3D_Scaffold_test/lib/python3.8/site-packages/matplotlib/mpl-data/fonts/ttf/
...
if check_conda_env('3D_Scaffold_test'):
    remove_conda_env('3D_Scaffold_test')
    print(f"Conda environment '3D_Scaffold_test' is removed, now is re-creating it.")
else:
    print(f"Conda environment '3D_Scaffold_test' does not exist, now is creating it.")



os.system(f"conda create -n 3D_Scaffold_test python=3.8 numpy=1.23.5 pytorch=1.5.1 torchvision cudatoolkit=10.2 ase=3.19.0 openbabel=3.1.1 rdkit=2019.09.2.0 requests matplotlib seaborn scikit-learn -c pytorch -c openbabel -c defaults -c conda-forge")
os.system(f"source $(conda info --base)/etc/profile.d/conda.sh")
os.system(f"conda run -n 3D_Scaffold_test pip install 'schnetpack==0.3'")
os.system(f"cp ../times*.ttf ~/anaconda3/envs/3D_Scaffold_test/lib/python3.8/site-packages/matplotlib/mpl-data/fonts/ttf/")
...
os.system(f"conda run -n 3D_Scaffold_test python ./duojincheng_obabel_smi_to_3D_sdf.py")   # parallel-convert the SMILES to the 3D sdf files
...
os.system(f"conda run -n 3D_Scaffold_test python ./bingfa_sdf_to_mol2.py")   # parallel-convert sdf to mol2 file and calculate the Gasteiger charge needed by AutoDock Vinna
...
os.system(f"conda run -n 3D_Scaffold_test python ./bingfa_mol2_to_pdbqt_by_obabel.py")   # convert the drug-like molecules to .pqbqt files for Vinna
os.system(f"conda run -n 3D_Scaffold_test python ./filter_small_55.py")    # optional; control the atom number of training molecules
os.system(f"conda run -n 3D_Scaffold_test python ./pdb_A_B_C_D_E_chains_pymol.py")   # optional; handle the target protein PDB residue ID
os.system(f"conda run -n 3D_Scaffold_test python ./pdb_duplicate_residue_remove.py")   # optional; handle the target protein residue multi-conformation in PDB
...
os.system(f"conda run -n 3D_Scaffold_test python ../extract_affinity.py") 
os.system(f"cp ./affinity.out ..")
os.chdir(f"../")
os.system(f"conda run -n 3D_Scaffold_test python ./sort_energy.py") 
...
os.system(f"conda run -n 3D_Scaffold_test python ./duojincheng_pdbqt_to_xyz_by_obabel.py")  # from the pdbqt to xyz_without_nonpolar_hydrogen to save GPU memory
os.system(f"conda run -n 3D_Scaffold_test python ./xyz_add_property.py")    # the molecules which not succeed in two docking processes will be filtered out in this step
os.system(f"conda run -n 3D_Scaffold_test python ./xyz_element_filter.py")  # control the element type of training molecules


######
###### convert the add-property .xyz file to the ase binary .db file format to accelerate the training
os.system(f"conda run -n 3D_Scaffold_test python ./generate_3D_Scaffold.py --xyz_path {absolute_work_dir_path}/ZINC_in-man_xyz_with_prop_elem_filter")  


######
######  pre-calculate the atom-pairs distance and save to accelerate the training
os.system(f"conda run -n 3D_Scaffold_test python ./zzx_preprocess_dataset.py ./world.db")   # pre-calculate the atom-pairs distance
os.system(f"conda run -n 3D_Scaffold_test python ./read_stat_npz.py ./world/worldgen_statistics.npz")  # show the preprocess_dataset logfile
os.system(f"conda run -n 3D_Scaffold_test python ./print_db.py")   # print and show the dataset
...
os.system(f"conda run -n 3D_Scaffold_test python {absolute_cG_SchNet_path}/gschnet_cond_script.py train gschnet ./data/ZINC_in-man/ ./models/ZINC_in-man/ --conditioning_json_path ./data/cond_Epro_Mpro.json --split 53000 1838 --cuda") 
...
os.system(f"conda run -n 3D_Scaffold_test python ./plot_log_complex.py")
...
os.chdir(f"{absolute_cG_SchNet_path}")
os.system(f"conda run -n 3D_Scaffold_test python {absolute_cG_SchNet_path}/gschnet_cond_script.py generate gschnet ./models/ZINC_in-man/ 3000 --conditioning \"Epro_affinity -8.0; Mpro_affinity -8.0\" --cuda")
...
if check_conda_env('MPerformer_test'):
    remove_conda_env('MPerformer_test')
    print(f"Conda environment 'MPerformer_test' is removed, now is re-creating it.")
else:
    print(f"Conda environment 'MPerformer_test' does not exist, now is creating it.")

os.system(f"conda create -n MPerformer_test python=3.9 pytorch=2.0 torchvision cudatoolkit=11.7 ase openbabel -c pytorch -c openbabel -c defaults -c conda-forge")
os.system(f"source $(conda info --base)/etc/profile.d/conda.sh")
...
os.system(f"conda run -n MPerformer_test pip install ./unicore-0.0.1+cu117torch2.0.0-cp39-cp39-linux_x86_64.whl")
os.system(f"conda run -n MPerformer_test pip install rdkit-pypi==2021.9.4")
os.system(f"conda run -n MPerformer_test pip install dpdata")
os.system(f"conda run -n MPerformer_test pip install torch==2.0 torchvision torchaudio")
os.system(f"conda run -n MPerformer_test pip install pandas")
os.system(f"conda run -n MPerformer_test pip install scikit-learn")
os.system(f"conda run -n MPerformer_test pip install numpy")
...
os.system(f"conda run -n 3D_Scaffold_test python ./mol_dict_to_xyz.py")
os.chdir(f"{absolute_Mperformer_path}/")
os.system(f"conda run -n MPerformer_test python ./predict.py --filename {absolute_Mperformer_path}/{newest_dir}/xyz --outputs_path {absolute_Mperformer_path}/{newest_dir}/sdf")
os.chdir(f"{absolute_Mperformer_path}/{newest_dir}")
os.system(f"conda run -n 3D_Scaffold_test python ./sdf_to_mol2Q_obabel.py")
os.system(f"conda run -n 3D_Scaffold_test python ./bingfa_mol2_to_pdbqt_by_obabel.py")
os.chdir(f"{absolute_work_dir_path}")
...
os.system(f"conda run -n 3D_Scaffold_test python plot_1d_prob_hebing.py 8 8")    # 8 8 are the generating conditions
os.system(f"conda run -n 3D_Scaffold_test python plot_3d_surface_prob.py 8 8")   # 8 8 are the generating conditions
os.system(f"conda run -n 3D_Scaffold_test python plot_3d_surface_prob_diff.py")
os.system(f"conda run -n 3D_Scaffold_test python plot_1d_freq.py")
os.system(f"conda run -n 3D_Scaffold_test python plot_3d_surface_freq.py")
os.system(f"conda run -n 3D_Scaffold_test python statistic_and+0.py")
os.system(f"conda run -n 3D_Scaffold_test python statistic_and+1.py")
os.system(f"conda run -n 3D_Scaffold_test python statistic_and+2.py")
os.system(f"conda run -n 3D_Scaffold_test python statistic_sum.py")
os.system(f"cp ../bingfa_mol2_to_smi.py .")
os.system(f"conda run -n 3D_Scaffold_test python bingfa_mol2_to_smi.py")
os.system(f"cp ../smi_to_warm_up.py .")
os.system(f"conda run -n 3D_Scaffold_test python smi_to_warm_up.py 0.66")    #  0.66 is warm_up value
os.system(f"conda run -n 3D_Scaffold_test python statistic_func_group_gro.py")
os.system(f"conda run -n 3D_Scaffold_test python statistic_func_group_mol.py")
os.system(f"conda run -n 3D_Scaffold_test python finger_print_plot.py")
os.system(f"conda run -n 3D_Scaffold_test python HDBSCAN.py")
os.system(f"conda run -n 3D_Scaffold_test python Plot_good_mol.py")
