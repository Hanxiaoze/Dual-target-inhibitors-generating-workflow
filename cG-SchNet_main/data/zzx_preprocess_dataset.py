...
from rdkit.Chem.Fragments import fr_Al_COO
...
def get_parser():
    ...
    main_parser.add_argument('--valence_list',
                             default=[1, 1, 5, 3, 6, 4, 7, 3, 8, 2, 9, 1, 12, 2, 13, 3, 14, 4, 15, 3, 16, 2, 17, 1], type=int,
                             nargs='+',
                             help='The valence of atom types in the form '
                                  '[type1 valence type2 valence ...] '
                                  '(default: %(default)s)')
    main_parser.add_argument('--n_threads', type=int, default=32,
                             help='Number of extra threads used while '
                                  'processing the data')
    main_parser.add_argument('--n_mols_per_thread', type=int, default=1,
...
def preprocess_molecules(mol_idcs, source_db, valence,
                         ...
                         invalid_list=None, print_progress=True):
    '''
    ...
    n_all = len(mol_idcs)
    #functional_group = Chem.MolFromSmiles("C=CC(=O)N")
    with connect(source_db) as source_db:
        ...
        for i in mol_idcs:
           ...
           ## m = Chem.MolFromSmiles(smiles_string_molecule)
            ...
            #atom_ids = []
            #for func_grp in fgs:
            #    fg_types.append(func_grp.type)
            #    atom_ids.append(func_grp.atomIds)

            #atomids = list(atom_ids[fg_types.index(functional_group)])
            ##atomids = list(m.GetSubstructMatch(functional_group))
            at = row.toatoms()
            # get positions and atomic numbers
            pos = at.positions
            numbers = at.numbers
            
            ##fun_grp_pos = np.array(pos)[atomids]
            ##fun_grp_atomic_nums = np.array(numbers)[atomids]
            ##print("fun_grp_pos",fun_grp_pos)
            ##print("fun_grp_atomic_nums",fun_grp_atomic_nums)
            ##centroid = centeroidnp(fun_grp_pos)
            ...
            file_lock.acquire()
            with connect("./worldgen.db") as new_db:
...
def preprocess_dataset(datapath, valence_list, n_threads, n_mols_per_thread=1,
                       logging_print=False, new_db_path=None, precompute_distances=True,
    ...
    if n_threads >= 1:
        ...
        for i in range(n_threads):
            ...
            threads += \
                [Process(target=_processing_worker,
                            name=str(i),
                            args=(qs_out[-1], qs_in[-1], lambda x: preprocess_molecules(x,
                                                    datapath,
                                                    valence,
        ...
        while np.any(working_flag == 1):
            ...
            for res in results:
                ...
                #for (at, data) in zip(mols, data_list):
                    #try:
                    #new_db.write(at, data=data)                     ########位置不对，放到 i 循环里试试
                    #except:
                        #continue
    ...
    # #print(stats_list)
    # headsss=get_count_statistics(get_stat_heads=True)
    # print(headsss, len(headsss))
    # for i in range(stats.shape[0]):
    #     print(headsss[i]+": "+str(stats_list[i]))
...
if __name__ == '__main__':
    os.system('rm ./worldgen.db')
    parser = get_parser()
    args = parser.parse_args()
    preprocess_dataset(**vars(args))


#os.system('cp {} {}'.format('scaffold3Dgen.db', 'scaffold3D.db'))
