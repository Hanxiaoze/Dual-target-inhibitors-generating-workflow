...
class QM9gen(DownloadableAtomsData):
    ...
    properties = [
        ...
        U0, U, H, G, Cv, 'n_atoms', 'relative_atomic_energy', 'Epro_affinity', 'Mpro_affinity'
    ]
    ...
    def __init__(self, path, subset=None, download=True, precompute_distances=True,
        ...
        self.path = path  # ./data/world/
        self.dbpath = os.path.join(self.path, f'qm9gen.db')  # 数据源
    ...
    def get_properties(self, idx):
        ...
        # recover connectivity matrix from compressed format  解压  连接矩阵
        # 数据库(压缩后的连接矩阵)      放入训练模型时(完整连接矩阵)
        ...
        for add_prop in self.load_additionally:
            if add_prop == 'fingerprint':
                fp = np.array(row.data['fingerprint'],
                              dtype=self.db_metadata['fingerprint_format'])
                properties['fingerprint'] = torch.FloatTensor(
    ...
    def _download(self):    # 重写了父类的初始化方法中的 _download() 方法，导致初始化时自动调用本方法
        works = True
        if not os.path.exists(self.dbpath):
            qm9_path = os.path.join(self.path, f'qm9.db')   # 如果路径下有 qm9.db，则不下载 ！！！！ 我直接利用这一点，在 ./data/world/ 路径下放一个伪 qm9.db
            if not os.path.exists(qm9_path):    # 如果路径下没有 qm9.db
                works = works and self._load_data() # 下载 gdb9.tar.gz ，解压至 gdb9_xyz，并用 gdb9_xyz 生成 qm9.db
            works = works and self._preprocess_qm9() # 对 qm9.db 预处理，invalid_list，调用 preprocess_dataset() 函数，计算 validity 和成对距离
        return works
    ...
    def _load_data(self):
        ...
        url = 'https://ndownloader.figshare.com/files/3195389'   # 看一下 xyz 文件的第二行属性值格式
        ...
        with connect(os.path.join(self.path, 'qm9.db')) as db:
            ...
            for i, xyzfile in enumerate(ordered_files):
                ...
                with open(xyzfile, 'r') as f:
                    lines = f.readlines()
                    l = lines[1].split()[2:]   # l 应该为数值
                    for pn, p in zip(self.properties, l):
                        properties[pn] = float(p) * self.units[pn]    # 属性 字典
                    with open(tmp, "wt") as fout:   
                        for line in lines:        # 属性栏数值也写进 tmp 中了 ！！！
                            fout.write(line.replace('*^', 'e'))

                with open(tmp, 'r') as f:
                    ats = list(read_xyz(f, 0))[0]  # 此处 ase.io.extxyz() 应该只过滤了原子坐标！！！
                db.write(ats, data=properties)     # 将 【坐标ats】 和 【属性properties】 写入 db
        logging.info('Done.')
    ...
    def _preprocess_qm9(self):
        ...
        if os.path.exists(raw_path):  # 如果本地路径下直接找到 qm9_invalid.txt
            ...
            invalid_list = np.loadtxt(raw_path)  # 有就直接加载 invalid_list
        else:
            ...
            try:
                ...
                request.urlretrieve(url.as_uri(), raw_path)    # 下载 qm9_invalid.txt
                logging.info('Done.')
                invalid_list = np.loadtxt(raw_path)    # 加载 invalid_list
            except HTTPError as e:
                logging.error('HTTP Error:', e.code, url)    # 不能下载 pre-computed list，通过 pre-processing 验证分子的 validity
        ...
        preprocess_dataset(datapath=qm9_db, valence_list=list(valence_list),       # 此处调用了 preprocess_dataset() 函数
